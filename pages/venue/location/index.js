...
const VenueLocation = () => {
  ...
  const fetchVenueEntityList = async () => {
    try {
      setIsLoading(true);
      const result = await axiosGet(apiRouter.VENUE_LOCATION_LIST);
      if (result.status) {
  ...
  const handleFormSubmit = async (val) => {
    ...
    try {
      setIsLoading(true);
      const result = await axiosPost(apiRouter.ADD_VENUE_LOCATION, insertData);
      if (result.status) {
        setIsEditId("");
  ...
  const handleItemDelete = async (val) => {
    ...
    try {
      ...
      const result = await axiosGet(apiRouter.REMOVE_VENUE_LOCATION + "/" + id);
      if (result.status) {
        fetchVenueEntityList();
        handleFormToggle(false);
      }
                ...
                <div className="col-lg-12">
                  <div className="ibox ">
                    <div className="ibox-title">
                      <h5>
                        Venue Location <small>Create</small>
                      </h5>
                    </div>
