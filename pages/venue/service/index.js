...
const VenueService = () => {
  ...
  const fetchVenueEntityList = async () => {
    try {
      setIsLoading(true);
      const result = await axiosGet(apiRouter.VENUE_SERVICE_LIST);
      if (result.status) {
  ...
  const handleFormSubmit = async (val) => {
    ...
    try {
      setIsLoading(true);
      const result = await axiosPost(apiRouter.ADD_VENUE_SERVICE, insertData);
      if (result.status) {
        setIsEditId("");
  ...
  const handleItemDelete = async (val) => {
    ...
    try {
      ...
      const result = await axiosGet(apiRouter.REMOVE_VENUE_SERVICE + "/" + id);
      if (result.status) {
        fetchVenueEntityList();
        handleFormToggle(false);
      }
                ...
                <div className="col-lg-12">
                  <div className="ibox ">
                    <div className="ibox-title">
                      <h5>
                        Venue Service <small>Create</small>
                      </h5>
                    </div>
